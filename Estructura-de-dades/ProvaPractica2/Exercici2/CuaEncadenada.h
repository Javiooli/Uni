...
class CuaEncadenada {
    public:
        ...
        bool iguals(const CuaEncadenada<T>& cuaC);
...
template <class T>
bool CuaEncadenada<T>::iguals(const CuaEncadenada<T>& cuaC) {
    bool iguals = true;
    int thisTamany = 0, CTamany = 0, iterator = 0;

    Node<T>* aux1 = this->_front->getNext();
    while (aux1 != nullptr) {
        thisTamany++;
        aux1 = aux1->getNext();
    }

    Node<T>* aux2 = cuaC._front->getNext();
    while (aux2 != nullptr) {
        CTamany++;
        aux2 = aux2->getNext();
    }

    iguals = (thisTamany == CTamany ? true : false);

    aux1 = _front->getNext();
    aux2 = cuaC._front->getNext();
    

    while (iguals && iterator < thisTamany) {
        if (aux1->getElement() != aux2->getElement()) iguals = false;
        else {
            aux1 = aux1->getNext();
            aux2 = aux2->getNext();
            iterator++;
